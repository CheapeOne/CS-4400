...
def register():
    ...
    if user == '''SELECT username from GTChatUsers where username = user''':
        ...
        return
    if passwd != confirm:
        messagebox.showerror('Error',"Registration Failed: Password does not mach confirmation")
        return
    for x in passwd:
        if x in digits:
            dig = dig+1
             ...
             Up = Up+1
    if dig==0 or Up==0:
        messagebox.showerror('Error',"Registration Failed: Password needs 1 capital letter and 1 number")
        return                      
toLoginwin()
